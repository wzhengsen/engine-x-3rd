...
static void storeatubox (lua_State* L, int lo)
{
	...
	lua_insert(L, -3);
	lua_settable(L, -3); /* on lua 5.3, we trade the "tolua_peers" lookup for a settable call */
	lua_pop(L, 1);
#else
...
static int class_index_event (lua_State* L)
{
    ...
    if (t == LUA_TUSERDATA)
    {
...
#if LUA_VERSION_NUM == 501 /* new macro on version 5.1 */
        lua_getfenv(L,1);
        if (!lua_rawequal(L, -1, TOLUA_NOPEER)) {
            lua_pushvalue(L, 2); /* key */
		...
		if (!lua_rawequal(L, -1, TOLUA_NOPEER)) {
			lua_pushvalue(L, 2); /* key */
			lua_gettable(L, -2); /* on lua 5.3, we trade the "tolua_peers" lookup for a gettable call */
			if (!lua_isnil(L, -1))
				return 1;
		};
#else
        ...
        {   /* stack: obj key obj mt */
            ...
            if (lua_isinteger(L,2))                 /* check if key is a numeric value */
            {
                lua_rawgeti(L, -1, lua_tointeger(L, 2));    /* stack: obj key mt value */
                if (!lua_isnil(L, -1))
                    return 1;
                else
                    lua_pop(L, 1);
...
static int class_newindex_event (lua_State* L)
{
    ...
    if (t == LUA_TUSERDATA)
    {
        ...
        lua_getuservalue(L,1);//uv value key userdata
        if (lua_istable(L,-1)) {
            lua_getfield(L, -1, ".cls.set");//.cls.set uv value key userdata
            if (!lua_isnil(L, -1)) {
                lua_pushvalue(L, 2);//key .cls.set uv value key userdata
                lua_rawget(L, -2);//ret .cls.set uv value key userdata
                if (lua_isfunction(L, -1)) {//function .cls.set uv value key userdata
                    lua_pushvalue(L, 1);//userdata function .cls.set uv value key userdata
                    lua_pushvalue(L, 3);//value userdata function .cls.set uv value key userdata
                    lua_call(L, 2, 0);//.cls.set uv value key userdata
                    return 0;
                }
            }
        }
        lua_settop(L, 3);//value key userdata
...
static int class_idiv_event(lua_State* L)
{
	return do_operator(L, ".idiv");
}
...
static int class_band_event(lua_State* L)
{
	return do_operator(L, ".band");
}

static int class_bor_event(lua_State* L)
{
	return do_operator(L, ".bor");
}

static int class_bxor_event(lua_State* L)
{
	return do_operator(L, ".bxor");
}

static int class_bnot_event(lua_State* L)
{
	if (lua_isuserdata(L, 1))
	{
		/* Try metatables */
		lua_pushvalue(L, 1);                     /* stack: op1 op1 */
		while (lua_getmetatable(L, -1))
		{   /* stack: op1 op1 mt */
			lua_remove(L, -2);                      /* stack: op1 mt */
			lua_pushstring(L, ".bnot");             /* stack: op1 mt key */
			lua_rawget(L, -2);                      /* stack: obj mt func? */
			if (lua_isfunction(L, -1))
			{
				lua_pushvalue(L, 1);				/* stack: obj mt func obj */
				lua_call(L, 1, 1);					/* stack: obj mt ret */
				return 1;
			}
			lua_settop(L, 2);
		}
...
static int class_shl_event(lua_State* L)
{
	return do_operator(L, ".shl");
}

static int class_shr_event(lua_State* L)
{
	return do_operator(L, ".shr");
}
...
TOLUA_API void tolua_classevents (lua_State* L)
{
	...
	lua_pushcfunction(L, class_idiv_event);
	lua_rawset(L, -3);
	...
	lua_pushcfunction(L, class_band_event);
	lua_rawset(L, -3);
	lua_pushstring(L, "__bor");
	lua_pushcfunction(L, class_bor_event);
	lua_rawset(L, -3);
	lua_pushstring(L, "__bxor");
	lua_pushcfunction(L, class_bxor_event);
	lua_rawset(L, -3);
	lua_pushstring(L, "__bnot");
	lua_pushcfunction(L, class_bnot_event);
	lua_rawset(L, -3);
	lua_pushstring(L, "__shl");
	lua_pushcfunction(L, class_shl_event);
	lua_rawset(L, -3);
	lua_pushstring(L, "__shr");
	lua_pushcfunction(L, class_shr_event);
	lua_rawset(L, -3);
